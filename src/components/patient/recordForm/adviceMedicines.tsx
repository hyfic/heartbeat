...
export const AdviceMedicines: React.FC = () => {
  const { loading, record, setRecord } = useRecordStore();
  ...
  const addMedicine = (e: any) => {
    ...
    let medicineDuration = `${duration} ${durationType}${
      Number(duration) != 1 ? 's' : ''
    }`;
      ...
      <Table variant='simple'>
        ...
        <Tbody>
          <>
            ...
            <Tr>
              <Td>{(record.medicines?.length || 0) + 1}</Td>
              <Td>
                <TextInput
                  disableTitle
                  title='Medicine'
